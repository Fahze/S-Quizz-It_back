...
class QuestionService {
  ...
  async getClassement(joueurs: JoueurClassement[]) {
    ...
    const { data: joueursExistants, error } = await supabase.from('profile').select('id, pseudo, elo');
    ...
    for (let i = 0; i < classement.length; i++) {
      ...
      const { error: updateError } = await supabase.from('profile').update({ elo: nouveauElo }).eq('id', joueur.idJoueur);
      ...
      const { error: insertError } = await supabase.from('historiquePartie').insert({
